...
```{r echo = FALSE}
gt::gt(dplyr::filter(one_sample_cal_tbl$res_summary, prop_pos_null >= 0.05, 
...
```{r eval = FALSE}
plot(one_sample_cal_tbl, type1_range = c(0.05, 0.1), minimum_power = 0.7)
```
...
*Note that when points were tied, the point with the highest posterior and predictive threshold was selected for plotting*

We can also use the function `optimize_design` to list the design options with optimal criteria with respect to Type I error/power and average sample size within the specified range of acceptable type I error and minimum power. Based on the clinical context of the trial design, one of these designs can be selected for use.
...
And we see that we can select from between two design choices. Both have posterior threshold 0.93. To optimize the distance to the top left point on the type I error by power plot, we would use a predictive threshold of 0.1, resulting in a  design with type I error of 0.08 and power of 0.893. To optimize the distance to the point with minimum average sample size under the null and maximum average sample size under the alternative, we would use a predictive threshold of 0.15, resulting in a design with type I error of 0.069 and power of 0.784. 
