...
*Note that overlapping points are jittered on both plots, so hover carefully to identify optimal points, or use 'optimize_design' to print the optimal design options*
...
We see that in this case a number of designs can minimize the distance to (0, 1) on the type I error by power plot, and likewise a number of designs can minimize the distance to the point with minimum average sample size under the null and maximum average sample size under the alternative.

In this case, the investigator would have to consider the tradeoffs in type I error/power versus average sample size to select the ideal design for their purposes.
