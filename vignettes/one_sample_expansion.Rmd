...
```{r echo = FALSE}
gt::gt(print(cal_tbl, type1_range = c(0.05, 0.1), minimum_power = 0.7))
```
...
But which design should we choose? The function `optimize_design` will return the design options that maximum sensitivity, specificity, and Youden's index within the specified range of acceptable type I error and minimum power. Based on the clinical context of the trial design, one of these designs can be selected for use.

```{r}
optimize_design(cal_tbl, type1_range = c(0.05, 0.1), minimum_power = 0.7)
```

Finally, we can plot the results to visualize the different design options.
