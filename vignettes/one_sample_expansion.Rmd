...
plot_tbl <- 
  cal_tbl %>% 
  filter(prop_pos_null >= lower_type1 & 
           prop_pos_null <= upper_type1 &
           prop_pos_alt >= min_power) %>% 
  mutate(
    `Youden's index` = prop_pos_alt + prop_stopped_null - 1
  )

p <- ggplot(plot_tbl, aes(x = prop_stopped_null, y = prop_pos_alt, 
...
simon_lower <- clinfun::ph2simon(pu = 0.1, pa = 0.3, ep1 = 0.05, ep2 = 0.25)
simon_upper <- clinfun::ph2simon(pu = 0.1, pa = 0.3, ep1 = 0.1, ep2 = 0.25)

xopt_lower <- simon_lower$out[
  c(((1:n)[simon_lower$out[, 5]==min(simon_lower$out[,5])])[1], 1), ]
plot_simon_lower <- tibble::add_column(
  dplyr::rename(
    tibble::as_tibble(xopt_lower), mean_n1_null = "EN(p0)", prop_stopped_null = "PET(p0)"
  ),
  prop_pos_null = 0.05,
  design = c("Simon's optimal \n Type I error = 0.05", 
             "Simon's minimax \n Type I error = 0.05"))
...
plot_simon_upper <- tibble::add_column(
  dplyr::rename(
    tibble::as_tibble(xopt_upper), mean_n1_null = "EN(p0)", prop_stopped_null = "PET(p0)"
  ),
  prop_pos_null = 0.05,
  design = c("Simon's optimal \n Type I error = 0.1", 
             "Simon's minimax \n Type I error = 0.1"))

p2 <- ggplot(plot_tbl, aes(x = mean_n1_null, y = prop_stopped_null, 
                           color = prop_pos_null)) + 
  geom_point() + 
  ylim(0, 1) + 
  labs(
  ...
  geom_point(data = plot_simon_lower) +
  geom_text(
    data = plot_simon_lower,
    aes(label = design),
    size = 3
  ) +
  geom_point(data = plot_simon_upper) +
  geom_text(
