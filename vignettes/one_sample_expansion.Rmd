...
The inputs to `calibrate_thresholds()` for our case study re-design include `prob_null = 0.1` as the null response rate, `prob_alt = 0.2` as the alternative response rate, `n = seq(5, 95, 5)` indicates interim looks after every 5 patients up to a total of 95, `direction = "greater"` specifies that interest is in whether the alternative response rate exceeds the null response rate, `p0 = 0.1` specifies the target response rate to compare to (this will be the same as `prob_null` in this case), `delta = NULL` because this argument specifies the clinically meaningful difference between groups, which is not relevant in the one-sample case, `prior = c(0.5, 0.5)` specifies that both hyperparameters of the prior beta distribution be set to 0.5, `S = 5000` specifies that 5000 posterior samples will be drawn to calculate the posterior and predictive probabilities, `N = 95` indicates the final total sample size of 95, `nsim = 1000` specifies that we will generate 1000 simulated datasets under both the null and the alternative, `pp_threshold` is a vector of the posterior thresholds of interest, and `ppp_threshold` is a vector of predictive thresholds of interest.
...
one_sample_cal_tbl <- 
  calibrate_thresholds(prob_null = 0.1, prob_alt = 0.2, n = seq(5, 95, 5),
                       direction = "greater", p0 = 0.1, delta = NULL, 
                       prior = c(0.5, 0.5), S = 5000, N = 95, nsim = 1000,
...
We will limit our consideration to designs with type I error between 0.01 and 0.2, and a minimum power of 0.7.
...
```{r echo = FALSE}
# I need to manually include the plots because otherwise they don't display properly in the vignette
...
The optimal accuracy design is the one with posterior threshold 0.9 and predictive threshold 0.05. It has a type I error of 0.072, power of 0.883, average sample size under the null of 51, and average sample size under the alternative of 91. 
