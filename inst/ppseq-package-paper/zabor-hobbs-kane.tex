...
predictive probability design using the \pkg{ppseq} package. While the
\pkg{ppseq} package was developed with early phase oncology clinical
...
well as the maximum total sample size \code{N}. Default arguments are
...
and the number of simulated trial datasets \code{nsim}, but
customization is also possible. The additional argument \code{delta} can
...
element of which is a \code{tibble} containing the posterior threshold,
...
also columns for \code{r0} and \code{r1}, the number of responses in the
...
associated with that decision \code{ppp}.
